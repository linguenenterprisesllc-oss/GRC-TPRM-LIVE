...
if st.session_state.stage == 0:
    ...
    if st.session_state.irq_submitted:
        ...
        a["record_count"]         = st.selectbox("4. Number of records at risk", ["— select —"] + RECORD_OPTS)
...
{ONETRUST_PROFILE}
Questionnaires sent:\n{q_list}\nDocument requests:\n{d_list}
Generate 6-8 realistic Q&A pairs per questionnaire. Mix strong answers with 1-2 gaps.
SOC 2 Type II, ISO 27001, DPA, Pen Test should be Received. COI and PCI DSS should be Not Provided.
Respond ONLY in valid JSON (no markdown):
{{"questionnaire_responses":[{{"name":"name","responses":[{{"question":"q","answer":"a","hasGap":false,"gapNote":""}}]}}],"document_status":[{{"name":"name","status":"Received or Not Provided","note":"brief note"}}]}}"""
            try:
                st.session_state.vendor_responses = parse_json(call_claude(prompt, 3000))
